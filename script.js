...
const projectsData = [
    {   
        ...
        details: [   
            "Constructed a robust sales forecasting model using **Facebook Prophet**, achieving a verified **92% Mean Absolute Percentage Error (MAPE)** for quarterly sales predictions. This precision directly facilitated strategic inventory management, effectively reducing stockouts by a demonstrable 10% and optimizing supply chain efficiency.",   
            "Engineered comprehensive, scalable data processing pipelines utilizing **Pandas and NumPy**. These pipelines were meticulously designed for robust ETL (Extract, Transform, Load) operations, preparing raw e-commerce data for complex time series forecasting and advanced business intelligence analysis, ensuring data integrity and readiness.",   
            "Created interactive and dynamic dashboards using **Plotly Express and Streamlit**. These dashboards provided real-time visualization of key e-commerce metrics and granular forecast performance, empowering stakeholders with actionable insights. This led to a **15% increase in the impact of data-driven revenue strategies** and a **5% uplift in overall quarterly sales revenue**, highlighting direct business value."   
        ],
    ...
    {   
        ...
        details: [   
            "Engineered and deployed a sophisticated real-time AI-driven algorithmic trading system on **Streamlit**. This platform provided live market data feeds, dynamic technical indicator computations, and automated trade execution capabilities, culminating in a **demonstrable 5% increase in simulated portfolio returns** under various market conditions.",   
            "Developed an enhanced, multi-factor trading strategy by rigorously combining **Moving Average Convergence Divergence (MACD), Relative Strength Index (RSI), and Bollinger Bands**. This comprehensive approach enabled advanced signal generation, validated with concurrent volume confirmation, to identify high-probability trading opportunities.",   
            ...
            "Integrated seamlessly with the **Alpaca API** for high-fidelity fetching of historical and live stock data. This integration facilitated real-time bar updates, enabling sub-second decision-making, and guaranteed reliable, low-latency order submission to the market."   
        ],   
    ...
    {   
        ...
        details: [   
            "Architected and successfully deployed a comprehensive, integrated Multi-Modal AI Toolkit on **Hugging Face Spaces**. This platform meticulously integrated 9 state-of-the-art Transformer pipelines, offering a diverse array of advanced AI capabilities including complex NLP tasks, computer vision applications, and robust speech processing.",   
            "Developed a resilient and high-performance **FastAPI** backend, leveraging asynchronous operations for maximal concurrency and **Pydantic models** for stringent data validation. This backend exposed modular AI services via well-defined `/api` endpoints, ensuring robust and reliable interaction.",   
            "Designed and implemented an intuitive **Streamlit** frontend, providing an interactive user interface for seamless AI service interaction. The frontend featured real-time API call history tracking and dynamic system status monitoring, enhancing user transparency and control.",   
            "Integrated advanced functionalities such as Text-to-Speech (TTS) with dynamic speaker embeddings for personalized audio output and Speech-to-Text (STT) with automatic audio resampling for optimal transcription accuracy. These enhancements demonstrably improved accessibility by **15% for over 5,000 daily users**, significantly broadening the toolkit's utility."   
        ],   
    ...
    {   
        ...
        details: [   
            "Developed and successfully deployed an integrated Hybrid Predictive Maintenance system on **Streamlit**. This innovative platform seamlessly combined supervised learning, specifically **Long Short-Term Memory (LSTM) networks**, with advanced reinforcement learning techniques to generate optimal, data-driven maintenance recommendations for industrial assets.",   
            "Engineered a sophisticated LSTM-based deep learning model using **TensorFlow/Keras** to precisely predict machine health degradation and Remaining Useful Life (RUL) from complex synthetic time-series sensor data. This model achieved a **30% improvement in prediction accuracy** compared to traditional methods, enhancing proactive maintenance capabilities.",   
            ...
            "Established a persistent **SQLite database** for rigorous logging of simulation reports, encompassing critical health metrics and interpretable insights (simulated **SHAP/LIME** values). This comprehensive logging strategy is estimated to **reduce operational downtime costs by 20%**, projecting annual savings of approximately $50,000 through optimized maintenance scheduling and reduced unexpected failures."   
        ],   
    ...
    {   
        ...
        details: [   
            "Architected and deployed a highly integrated Customer Churn Prediction system, featuring a **Streamlit** frontend for interactive user engagement and a **FastAPI** backend for robust model inference. This end-to-end solution provides real-time insights into customer attrition risks.",   
            "Developed a robust churn prediction model, achieving a notable **F1-score of 0.87**. This model, utilizing advanced machine learning techniques, demonstrated high efficacy in identifying approximately **75% of potential churners within a crucial 30-day window**, enabling proactive retention strategies.",   
            "Engineered sophisticated data preprocessing pipelines, strategically incorporating **SMOTE (Synthetic Minority Over-sampling Technique)** to address inherent class imbalance in the dataset. This technique demonstrably increased the model's recall for the minority (churn) class by **25%**, significantly enhancing its ability to detect at-risk customers.",   
            "Launched a high-performance RESTful API using **FastAPI and Uvicorn**, engineered for optimal speed and scalability. This API achieved sub-100ms inference latency and was rigorously tested to handle up to **500 requests per second**, ensuring efficient real-time prediction capabilities for large-scale applications."   
        ],   
...
const blogPostsData = [
    {   
        ...
        content: `
            ...
            <p>From a foundational perspective, the entire application is architected around a robust web development stack. HTML provides the semantic structure, ensuring accessibility and search engine optimization. CSS, augmented by the utility-first framework **Tailwind CSS**, enables rapid and consistent styling, allowing for a highly responsive and aesthetically pleasing user interface across various devices. The dynamic and interactive elements that bring this portfolio to life are powered by pure **JavaScript**, demonstrating proficiency in client-side scripting and DOM manipulation.</p>
    ...
    {   
        ...
        content: `
            ...
            <p>My academic trajectory and professional engagements have been rigorously focused on acquiring mastery across the entire machine learning lifecycle. This encompasses the meticulous construction of resilient data pipelines, leveraging distributed computing paradigms such as **Hadoop and Spark**, to the strategic deployment of scalable microservices powered by **Python and FastAPI**. My objective consistently revolves around engineering robust, end-to-end systems that are not only performant but also maintainable and extensible.</p>
            <p>My internship as a Python Backend Developer served as an exceptional proving ground, enabling me to refine my competencies in advanced API development, efficient database management with **PostgreSQL**, and streamlined Docker-based deployments. This practical experience was instrumental in contributing to significant enhancements in system performance and operational efficiency within a production environment.</p>
            ...
            <p>Beyond the foundational backend infrastructure, my core intellectual curiosity resides in the intricate dynamics of the models themselves. I have immersed myself in a diverse spectrum of machine learning paradigms, ranging from conventional predictive analytics, such as the customer churn models meticulously constructed using **Random Forests**, to the avant-garde frontiers of deep learning, incorporating architectures like **Convolutional Neural Networks (CNNs)**, **Long Short-Term Memory (LSTMs)**, and **Transformers**. A project that I hold in particularly high esteem is the **AI Services Toolkit Pro (Multi-Modal AI Assistant)**, a complex integration where I synergized OpenAI's Whisper for state-of-the-art transcription capabilities with YOLOv5 for real-time object detection. The technical challenge of seamlessly merging these disparate yet powerful technologies into a singular, cohesive, and user-centric application was exhilarating and deeply rewarding.</p>
            <p>My commitment extends beyond mere model development to ensuring that these intelligent systems are not only robustly engineered but also demonstrably scalable and inherently maintainable. This necessitates a stringent adherence to **MLOps** principles, encompassing the meticulous implementation of Continuous Integration/Continuous Deployment (CI/CD) pipelines, comprehensive model versioning strategies, and diligent continuous monitoring protocols. My hands-on experience with **Docker** and platform-as-a-service providers like **Render and Heroku** has been pivotal in facilitating the efficient deployment of these intrinsically complex systems. It is my unwavering conviction that a meticulously engineered deployment strategy is as critically important as the predictive or generative power of the model itself in delivering tangible business value.</p>
            <p>I am particularly enthused by the rapid advancements in **Generative AI and Large Language Models (LLMs)**, and I have proactively engaged in exploring their transformative applications, as evidenced by my specialized certification in this cutting-edge domain. The inherent capacity of these technologies to autonomously create novel content, synthesize vast quantities of information, and even generate executable code unlocks an unprecedented array of possibilities for the next generation of AI solutions, and I am eager to contribute to their evolution.</p>
        `   
    },
    {   
    ...
    },
    {
        ...
        content: `
            <p>In the high-octane and perpetually evolving domain of financial trading, the acquisition and judicious utilization of real-time data transcend mere advantage; it constitutes an absolute strategic imperative. My **AI-Powered Trading System with Risk Analytics** project was meticulously engineered around this fundamental principle, demanding the construction of highly robust and ultra-low latency data ingestion pipelines. We leveraged distributed streaming technologies, notably **Apache Kafka**, to process an astounding throughput of **over 10,000 data points per minute**, ensuring immediate availability of market intelligence.</p>
            ...
            <p>The paramount engineering challenge revolved around guaranteeing minimal data latency and maximizing throughput while maintaining data integrity. By architecting a solution around distributed streaming platforms, we successfully established a resilient, highly efficient, and fault-tolerant backbone for our intricate predictive models. This project unequivocally demonstrated that superior data engineering is not merely supportive but absolutely foundational to the successful implementation and sustained performance of advanced AI applications within the demanding landscape of quantitative finance. Our strategic integration with the **Alpaca API** further solidified this capability by providing streamlined and robust access to both historical and real-time stock data, enabling granular bar updates and facilitating seamless, high-frequency order submissions directly to trading venues.</p>
        `   
    },
    {
        ...
        content: `
            <p>As Artificial Intelligence models escalate in complexity, particularly within mission-critical applications such as predictive maintenance, comprehending the underlying rationale for a model's prediction becomes as strategically vital as the prediction itself. This paradigm defines the burgeoning field of Explainable AI (XAI). In my **Hybrid Predictive Maintenance System**, a core design objective was to demystify the inherently complex interplay of Deep Learning and Reinforcement Learning, rendering the model's decisions transparent and actionable through the strategic application of **SHAP (SHapley Additive exPlanations) and LIME (Local Interpretable Model-agnostic Explanations)**.</p>
    ...
    },
    {
        ...
        content: `
            <p>The contemporary landscape of Artificial Intelligence increasingly necessitates systems capable of processing and synthesizing information across disparate data modalities – text, speech, and images. Building and deploying state-of-the-art AI models that seamlessly handle this multi-modal complexity presents considerable engineering challenges. My **AI Services Toolkit Pro (Multi-Modal AI Assistant)** project was conceived and meticulously designed to address these challenges head-on, by establishing a unified, extensible platform for a diverse array of Transformer-based AI capabilities.</p>
            <p>The core architectural principle was the strategic integration of **9 distinct Transformer pipelines sourced from Hugging Face**, a leading ecosystem for pre-trained models. This integration encompassed a broad spectrum of AI tasks, including nuanced sentiment analysis, concise text summarization, precise image captioning, and advanced question-answering. This modular approach enabled the offering of a wide range of sophisticated AI services from a singular, cohesive, and high-performance application endpoint.</p>
            <p>Key aspects of the system's robust architecture include:</p>
            <ul>
                <li>A high-performance **FastAPI backend** meticulously engineered to handle asynchronous operations, ensuring maximal concurrency and responsiveness. This backend leverages **Pydantic models** for rigorous data validation and serialization, guaranteeing data integrity and type safety. AI services are exposed via clean, RESTful \`/api\` endpoints, facilitating easy integration with other systems.</li>
                <li>An intuitive and dynamic **Streamlit frontend** that provides a rich, interactive user experience. This interface allows users to seamlessly interact with various AI services, observe real-time processing feedback, review comprehensive API call histories, and monitor overall system status, enhancing user transparency and control.</li>
                <li>Implementation of advanced multi-modal features, including sophisticated **Text-to-Speech (TTS)** capabilities with dynamic speaker embeddings for personalized voice output, and highly accurate **Speech-to-Text (STT)** with automatic audio resampling to optimize transcription quality across diverse audio inputs. These accessibility enhancements demonstrably improved usability for an estimated **5,000 daily users**, significantly broadening the toolkit’s appeal and utility.</li>
            </ul>
            <p>This project serves as a comprehensive demonstration of a full end-to-end MLOps pipeline, spanning from intricate model integration and robust API development to seamless frontend deployment on scalable platforms such as **Hugging Face Spaces**. It unequivocally highlights the transformative power of strategically combining specialized AI models into a user-friendly, enterprise-ready toolkit, thereby rendering advanced AI capabilities both highly accessible and readily actionable for a diverse user base.</p>
        `
    },
    {
        ...
        content: `
            ...
            <ul>
                <li><strong>Continuous Integration/Continuous Delivery (CI/CD):</strong> This pillar involves automating the entire pipeline for building, rigorously testing, and reliably deploying models. My direct involvement in orchestrating **Docker-containerized deployments to platforms like Render and Heroku** demonstrably reduced critical deployment cycles by a remarkable **40%**. This automation minimizes manual errors, accelerates iteration velocity, and ensures faster time-to-market for AI-driven features.</li>
                ...
                <li><strong>Scalable Infrastructure Design:</strong> Architecting systems that can seamlessly handle escalating inference loads and data volumes is fundamental for production AI. My direct experience in building scalable microservices using **FastAPI and Flask** resulted in an impressive **25% improvement in API response times** and demonstrated the capability to handle **over 10,000 daily requests with an exceptional 99.9% uptime**, a clear testament to robust architectural design and effective load balancing.</li>
            </ul>
            <p>Furthermore, the advent of platforms like **Hugging Face Spaces** significantly democratizes the application of MLOps principles, providing accessible and efficient avenues for deploying, sharing, and collaborating on machine learning models. By diligently mastering these MLOps tenets, AI solutions transcend the realm of mere innovative prototypes; they evolve into reliable, high-performing, and continuously valuable assets that deliver sustained strategic advantages and profound business impact.</p>
        `   
    },
    {
        ...
        content: `
            <p>At the foundational core of every highly effective Artificial Intelligence or Machine Learning system lies an undeniably robust, meticulously engineered, and perpetually reliable data pipeline. Without a continuous flow of clean, accessible, and high-fidelity data, even the most exquisitely sophisticated models are ultimately rendered impotent. My academic background, coupled with extensive hands-on project experience, has equipped me with profound expertise in architecting and implementing **resilient data engineering solutions** that serve as the lifeblood of intelligent systems.</p>
            <p>My specialized certifications in Data Engineering, particularly focusing on **Hadoop & Spark**, provided the essential theoretical and practical groundwork for comprehending and managing large-scale data processing paradigms. Key technologies that form the bedrock of modern AI-driven data pipelines are:</p>
            <ul>
                <li>**Apache Hadoop:** A foundational framework for distributed storage (HDFS) and batch processing of colossal datasets. Hadoop's architecture enables the handling of petabyte-scale data, making it indispensable for foundational data lakes and historical data analysis for model training.</li>
                <li>**Apache Spark:** A powerful and versatile unified analytics engine designed for large-scale data processing. Spark offers unparalleled speed and flexibility, making it ideal for a multitude of tasks including complex ETL (Extract, Transform, Load) operations, advanced analytics, and machine learning feature engineering on massive datasets, significantly outperforming traditional MapReduce in many scenarios.</li>
                <li>**Apache Kafka:** A distributed streaming platform architected for building real-time data pipelines and streaming applications. In my **AI-Powered Trading System with Risk Analytics**, I leveraged Apache Kafka to establish ultra-low latency, real-time data ingestion pipelines capable of processing **over 10,000 data points per minute**. This real-time capability was paramount, significantly boosting decision efficiency and enabling immediate responsiveness to market dynamics.</li>
                <li>**SQL and NoSQL Databases:** Proficiency extends to proficiently managing and optimizing various database systems, including relational databases like **PostgreSQL and MySQL** for structured data integrity and consistency, and NoSQL databases such as **MongoDB** for flexible, scalable storage of unstructured or semi-structured data. My work specifically utilizing the **SQLAlchemy ORM** in conjunction with PostgreSQL databases resulted in a **quantifiable reduction of data retrieval time by 15%**, optimizing application performance and responsiveness.</li>
            </ul>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('mousemove', (e) => { mouse.x = e.x; mouse.y = e.y; }); // Corrected: e.y was just e.y, now it's e.y = e.y
    class Particle { constructor(x, y, dx, dy) { this.x = x; this.y = y; this.directionX = dx; this.directionY = dy; this.size = (Math.random() * 2) + 1; } draw() { ctx.beginPath(); ctx.arc(this.x, this.y, this.size, 0, Math.PI * 2); ctx.fillStyle = 'rgba(139, 92, 246, 0.2)'; ctx.fill(); } update() { if (this.x > bgCanvas.width || this.x < 0) this.directionX = -this.directionX; if (this.y > bgCanvas.height || this.y < 0) this.directionY = -this.directionY; let dx = mouse.x - this.x; let dy = mouse.y - this.y; if (Math.hypot(dx, dy) < mouse.radius + this.size) { if (mouse.x < this.x && this.x < bgCanvas.width - this.size * 10) this.x += 5; if (mouse.x > this.x && this.x > this.size * 10) this.x -= 5; if (mouse.y < this.y && this.y < bgCanvas.height - this.size * 10) this.y += 5; if (mouse.y > this.y && this.y > this.size * 10) this.y -= 5; } this.x += this.directionX; this.y += this.directionY; this.draw(); } } // Corrected: Full closing braces and draw call
